...
if(makeOutputDB) source("PostgreSQL_CreateDB_Tables.R", echo=F, keep.source=F)
		...
		if( todo$aggregate ){
			...
			for (sc in 1:scenario_No){
				...
				if(any(create_treatments == "Exclude_ClimateAmbient") && i_sw_input_treatments$Exclude_ClimateAmbient && sc==1) {
					...
					if(makeOutputDB) {
						header[is.na(header)] <- 0
						dbSendQuery(con, paste("INSERT INTO \"Aggregation_Overall\" VALUES (",paste("'",header,"'",sep="", collapse=","),");", sep=""))
				...
				if((!continueAfterAbort | (continueAfterAbort & !isdone.overallAggs[sc])) && !Exclude_ClimateAmbient){
					...
					if(makeOutputDB) {
						header[is.na(header)] <- 0
						SQL_INSERT <- paste("INSERT INTO \"Aggregation_Overall\" VALUES ('",((i-1)*scenario_No+sc),"',",paste("'",header,"'",sep="", collapse=","),");", sep="")
					}
					
					#1. Fractional Vegetation Composition
					if(aon$input_FractionVegetationComposition) {
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"input_FractionVegetationComposition\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",vec,"'",sep="", collapse=","), ");\n", sep=""))
						nv <- nv+3
					}
					...
					if(aon$input_VegetationPeak) {
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"input_VegetationPeak\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv:(nv+1)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+2
					}
					
					#3. Climate Perturbations
					if(aon$input_ClimatePerturbations) {
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"input_ClimatePerturbations\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv:(nv+35)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+36
					}
					#4. input Phenology # all growing?
					if(aon$input_Phenology & any(simulation_timescales=="monthly")) {
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"input_Phenology\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv:(nv+1)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+2
					}
					...
					if(any(simulation_timescales=="yearly") & aon$yearlyTemp){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"yearlyTemp\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv:(nv+1)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+2
					}
					...
					if(any(simulation_timescales=="yearly") & aon$yearlyPPT){			
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"yearlyPPT\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv:(nv+1)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+7
					}
					...
					if(any(simulation_timescales=="yearly") & aon$yearlyDryWetPeriods){			
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"yearlyDryWetPeriods\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv:(nv+1)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+2
					}
					#8. 
					if(any(simulation_timescales=="yearly") & aon$yearlyPET){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"yearlyPET\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv:(nv+1)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+2
					}
					#9. 
					if(any(simulation_timescales=="yearly") & any(simulation_timescales=="monthly") & aon$yearlymonthlyTemperateDrylandIndices){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"yearlymonthlyTemperateDrylandIndices\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv:(nv+8)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+9
					...
					if(any(simulation_timescales=="monthly") & aon$monthlyPlantGrowthControls){	#Nemani RR, Keeling CD, Hashimoto H et al. (2003) Climate-Driven Increases in Global Terrestrial Net Primary Production from 1982 to 1999. Science, 300, 1560-1563.
						...
						if(i==ifirst || makeOutputDB) resultfiles.Aggregates.header[nv:(nv+5)] <- c(temp <- c("Control_Temperature", "Control_Water", "Control_Radiation"), paste(temp, "_sd", sep=""))
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlyPlantGrowthControls\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv:(nv+5)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+6
					...
					if(any(simulation_timescales=="monthly") & aon$monthlySPEIEvents){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlySPEIEvents\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[start:(nv-1)],"'",sep="", collapse=","), ");", sep=""))
					}
					...
					if(any(simulation_timescales=="yearly") & aon$yearlyAET){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"yearlyAET\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv:(nv+1)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+2
					}	
					#13. 
					if(any(simulation_timescales=="monthly") & aon$monthlyTemp){
						...
						if(i==ifirst || makeOutputDB) resultfiles.Aggregates.header[nv:(nv+23)] <- c(paste("TempC_m", st_mo, sep=""), paste("TempC_m", st_mo, "_sd", sep=""))
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlyTemp\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[c(nv+st_mo-1,nv+st_mo-1+12)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+24
					}
					#14. 
					if(any(simulation_timescales=="monthly") & aon$monthlyPPT){
						...
						if(i==ifirst || makeOutputDB) resultfiles.Aggregates.header[nv:(nv+23)] <- c(paste("PPT_mm_m", st_mo, sep=""), paste("PPT_mm_m", st_mo, "_sd", sep=""))
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlyPPT\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[c(nv+st_mo-1,nv+st_mo-1+12)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+24
					}
					#15. daily response to weather generator treatments
					if(any(simulation_timescales=="daily") & aon$dailyWeatherGeneratorCharacteristics){
						...
						if(i==ifirst || makeOutputDB) resultfiles.Aggregates.header[nv:(nv+59)] <- c(paste("meanDWSdays_m", st_mo, sep=""), paste("meanDWSdays_m", st_mo, ".sd", sep=""), paste("meanDDSdays_m", st_mo, sep=""), paste("meanDDSdays_m", st_mo, "_sd", sep=""), paste("StDevDailyTempC_m", st_mo, sep=""))
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"dailyWeatherGeneratorCharacteristics\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[c(nv+st_mo-1,nv+st_mo-1+12,nv+st_mo-1+24,nv+st_mo-1+36,nv+st_mo-1+48)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+60
						
						rm(dws, dws.sd, dds, dds.sd, tv)
					}
					#16. daily weather frequency distributions
					if(any(simulation_timescales=="daily") & aon$dailyWeatherEventSizeDistribution){
						...
						if(i==ifirst || makeOutputDB) resultfiles.Aggregates.header[nv:(nv+13)] <- c(temp <- paste("PrcpEventSizes_NoPerYearIn", bins.summary, "+mm", sep=""), paste(temp, "_sd", sep=""))
						...
						if(i==ifirst || makeOutputDB) resultfiles.Aggregates.header[(nv+14):(nv+21)] <- c(temp <- paste("PrcpFreeDurations_NoPerYearIn", bins.summary+1, "+days", sep=""), paste(temp, "_sd", sep=""))
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"dailyWeatherEventSizeDistribution\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[c(nv:(nv+6),(nv+7):(nv+7+6),(nv+14):(nv+14+3),(nv+14+4):(nv+14+4+3))],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+22
					}
					#17. Variables to estimate percent C4 species in North America: Teeri JA, Stowe LG (1976) Climatic patterns and the distribution of C4 grasses in North America. Oecologia, 23, 1-12.
					...
					if(any(simulation_timescales=="daily") & aon$dailyC4_TempVar){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"dailyC4_TempVar\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv:(nv+5)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+6
						
					}
					
					#18. mean and SD of DOY and value of minimum/maximum of transpiration as in'dailySWPextremes' (don't distinguish between top/bottom soil layers)
					if(any(simulation_timescales=="daily") & aon$dailyTranspirationExtremes) {
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"dailyTranspirationExtremes\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[(nv-4):(nv+3)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+4
						
						rm(extremes)
					}
					
					#19. mean and SD of DOY and value of minimum/maximum of total evaporation (surface + soil) as in'dailySWPextremes' (don't distinguish between top/bottom soil layers)
					if(any(simulation_timescales=="daily") & aon$dailyTotalEvaporationExtremes) {
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"dailyTotalEvaporationExtremes\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[(nv-4):(nv+3)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+4
						
						rm(extremes)
					}
					
					#20. mean and SD of DOY and value of minimum/maximum of deep drainage as in'dailySWPextremes' (don't distinguish between top/bottom soil layers
					if(any(simulation_timescales=="daily") & aon$dailyDrainageExtremes) {
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"dailyDrainageExtremes\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[(nv-4):(nv+3)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+4
						
						rm(extremes)
					}
					
					#21. mean and SD of DOY and value of minimum/maximum of AET as in'dailySWPextremes' (don't distinguish between top/bottom soil layers
					if(any(simulation_timescales=="daily") & aon$dailyAETExtremes) {						
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"dailyAETExtremes\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[(nv-4):(nv+3)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+4
					...
					if(any(simulation_timescales=="monthly") & aon$monthlySnowpack){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlySnowpack\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[c(nv+st_mo-1,nv+st_mo-1+12)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+24
					}
					
					#23. daily snowpack: accountNSHemispheres_agg
					if(any(simulation_timescales=="daily") & aon$dailySnowpack){
						...
						if(makeOutputDB & !any(simulation_timescales=="yearly")) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"dailySnowpack\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv:(nv+9)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+10
					}		
					#24. 
					if(any(simulation_timescales=="daily") & any(simulation_timescales=="yearly") & aon$dailySnowpack){			
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"dailySnowpack\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[(nv-10):(nv+1)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+2
						rm(rainOnsnow)
					}
					...
					if(any(simulation_timescales == "monthly") & aon$monthlySoilTemp) {
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlySoilTemp\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[c(nv+st_mo-1, nv+st_mo-1+12)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+24
					}
					...
					if(any(simulation_timescales=="monthly") & aon$monthlySWP){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlySWP\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[c(nv+st_mo-1, nv+st_mo-1+12)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+24
					}
					
					
					#27. dry periods based on monthly swp data: accountNSHemispheres_agg
					if(any(simulation_timescales=="monthly") & aon$monthlySWPdryness){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlySWPdryness\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[(start):(nv+4*length(SWPcrit_MPa)-1)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+4*length(SWPcrit_MPa)
					}
					
					
					#28. extremes and timing of swp on a monthly basis
					if(any(simulation_timescales=="monthly") & aon$monthlySWPextremes){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlySWPextremes\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[(nv-4):(nv+3)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+4	
					}
					
					
					#29. extremes and timing of swp on a daily basis
					if(any(simulation_timescales=="daily") & aon$dailySWPextremes){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"dailySWPextremes\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[(nv-8):(nv+7)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+8
					...
					#30. correl monthly swp (top and bottom) vs. pet and ppt vs. temp, use product moment correlation coefficient {eqn. 11.6, \Sala, 1997 #45}
					if(any(simulation_timescales=="monthly") & aon$monthlyCorrelations){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlyCorrelations\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv:(nv+5)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+6
					}
					
					#31. cummulative frequency distribution of durations of dry soils in each of the four seasons and for each of the SWP.crit
					if(any(simulation_timescales=="daily") & aon$dailySWPdrynessDurationDistribution){
						...
						for(icrit in seq(along=SWPcrit_MPa)){
							...
							for(season in 1:nrow(mo_seasons)){
								...
								if(i==ifirst || makeOutputDB) resultfiles.Aggregates.header[nv:(nv+2*length(deciles)-1)] <- c(paste("Q", formatC(deciles*100, flag="0", width=3, format="d"), "%_DDTSID_S_", season.flag[season], "_SWPc", paste(abs(round(-1000*SWPcrit_MPa[icrit], 0)), "kPa", sep=""), sep=""), paste("Q", formatC(deciles*100, flag="0", width=3, format="d"), "%_DDBSID_S", season.flag[season], "_SWPc", paste(abs(round(-1000*SWPcrit_MPa[icrit], 0)), "kPa", sep=""), sep=""))
								nv <- nv+2*length(deciles)
							}
						}
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"dailySWPdrynessDurationDistribution\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[start:(nv-1)],"'",sep="", collapse=","), ");", sep=""))
					...
					if(any(simulation_timescales=="daily") && aon$dailySWPdrynessEventSizeDistribution)
					{
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"dailySWPdrynessEventSizeDistribution\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[start:(nv-1)],"'",sep="", collapse=","), ");", sep=""))
					...
					if(any(simulation_timescales=="daily") && aon$dailySWPdrynessIntensity)
					{
						...
						for(icrit in seq(along=SWPcrit_MPa)){
							...
							if(i==ifirst || makeOutputDB) {
								baseTitle <- paste("DP_MW_", paste(abs(round(-1000*SWPcrit_MPa[icrit], 0)), "kPa_", sep=""), rep(c("top_", "bottom_"), each=4), rep(c("AS_mmH2O", "MEA_mmH2OperED", "MED_Days", "MEN_CPY"), times=2), sep="")
								resultfiles.Aggregates.header[nv:(nv+15)] <- paste(rep(baseTitle, each=2), c("mean", "sd"), sep=".")
							}
							nv <- nv+16
						}
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"dailySWPdrynessIntensity\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[start:(nv-1)],"'",sep="", collapse=","), ");", sep=""))
					...
					#34. Degree days based on daily temp, and wet degree days on daily temp and swp
					if(any(simulation_timescales=="daily") & aon$dailyDegreeDays){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"dailyDegreeDays\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv:(nv+2+6*length(SWPcrit_MPa)-1)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+2+6*length(SWPcrit_MPa)	
					}
					#35. Dry and wet periods based on daily swp: accountNSHemispheres_agg
					if(any(simulation_timescales=="daily") & aon$dailySWPdrynessANDwetness){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"dailySWPdrynessANDwetness\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv:(nv+2*16*length(SWPcrit_MPa)-1)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+2*16*length(SWPcrit_MPa)	
					}	
					
					
					#36. volumetric soil water content top and bottom for each month
					#monthly data
					if(any(simulation_timescales=="monthly") & aon$monthlyVWC){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlyVWC\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[c(nv+st_mo-1,nv+st_mo-1+12)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+24
					}				
					
					
					#37. total soil water content top and bottom for each month
					#monthly data
					if(any(simulation_timescales=="monthly") & aon$monthlySWC){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlySWC\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[c(nv+st_mo-1,nv+st_mo-1+12)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+24
					}				
					
					
					#38. available soil water top and bottom for each month
					#monthly data
					if(any(simulation_timescales=="monthly") & aon$monthlySWA){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlySWA\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[c(nv+st_mo-1,nv+st_mo-1+12)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+24
					}	
					
					#39. extremes and timing of swa
					if(any(simulation_timescales=="monthly") & aon$monthlySWAextremes){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlySWAextremes\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[(nv-4):(nv+3)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+4	
					}
					
					#40. transpiration top and bottom for each month
					#monthly data
					if(any(simulation_timescales=="monthly") & aon$monthlyTranspiration){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlyTranspiration\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[c(nv+st_mo-1,nv+st_mo-1+12)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+24
					}	
					
					
					#41. soil evaporation for each month
					#monthly data
					if(any(simulation_timescales=="monthly") & aon$monthlySoilEvaporation){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlySoilEvaporation\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv+st_mo-1],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+12
					}	
					
					
					#42. monthly AET and PET	
					if(any(simulation_timescales=="monthly") & aon$monthlyAET){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlyAET\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv+st_mo-1],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+12
					}	
					#43. 
					if(any(simulation_timescales=="monthly") & aon$monthlyPET){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlyPET\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv+st_mo-1],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+12
					}	
					
					
					#44. transpiration:PET and soil evaporation:PET for each month
					#monthly data
					if(any(simulation_timescales=="monthly") & aon$monthlyPETratios){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlyPETratios\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[c(nv+st_mo-1, nv+st_mo-1+12)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+24
					}	
					
					
					#45. transpiration:AET and soil evaporation:AET for each month
					#monthly data
					if(any(simulation_timescales=="monthly") & aon$monthlyAETratios){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlyAETratios\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[c(nv+st_mo-1, nv+st_mo-1+12)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+24
					}	
					#46. 
					if(any(simulation_timescales=="monthly") & aon$monthlyRunoff){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlyRunoff\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv+st_mo-1],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+12
					}
					
					#47. hydraulic redistribution top and bottom for each month
					#monthly data
					if(any(simulation_timescales=="monthly") & aon$monthlyHydraulicRedistribution){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlyHydraulicRedistribution\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[c(nv+st_mo-1,nv+st_mo-1+12)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+24
					}	
					
					
					#48. monthly Infiltration
					if(any(simulation_timescales=="monthly") & aon$monthlyInfiltration){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"monthlyInfiltration\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv+st_mo-1],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+12
					}	
					...
					if(any(simulation_timescales=="yearly")  & aon$yearlyWaterBalanceFluxes) {
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"yearlyWaterBalanceFluxes\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv:(nv+57)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+58
					}
					...
					if(any(simulation_timescales=="daily")  & aon$dailySWPbasedRegeneration) {
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"dailySWPbasedRegeneration\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[nv:(nv+1)],"'",sep="", collapse=","), ");", sep=""))
						nv <- nv+2
					}
					...
					if(any(simulation_timescales=="daily")  & aon$dailyRegeneration_byTempSWPSnow & no.species_regeneration > 0){
						...
						if(makeOutputDB) SQL_INSERT <- paste(SQL_INSERT, paste("INSERT INTO \"dailyRegeneration_byTempSWPSnow\" VALUES ('",((i-1)*scenario_No+sc),"','",i,"','",sc,"',", paste("'",res[start:(nv-1)],"'",sep="", collapse=","), ");", sep=""))
					}
					print("before write")
					...
					if(!makeOutputDB) write.csv(out.temp, file=filename.out.temp[sc], quote=FALSE, row.names=FALSE )
					#save(out.temp, file=file.path(dir.out, paste(mpi.comm.rank(), "_", sc, "dataForDatabase.robj", sep="")))
					if(makeOutputDB) {
						SQL_INSERT <- gsub(pattern="NA", replacement="NaN", x=SQL_INSERT)
						save(SQL_INSERT,file=file.path(dir.out, paste("sql_insert_Run_",i_labels,"_SC_",sc,".r",sep="")))
						dbSendQuery(con, SQL_INSERT)
						#dbWriteTable(con, "Aggregation_Overall_1", out.temp[,1:700], row.names=FALSE, append=TRUE)#mpi.send.Robj(out.temp, 1, 1, 1)#request=mpi.comm.rank()
						#dbWriteTable(con, "Aggregation_Overall_2", out.temp[,c(1:length(header),701:dim(out.temp)[2])], row.names=FALSE, append=TRUE)
				...
				if(any(simulation_timescales=="daily") && daily_no > 0){
					...
					for (doi in 1:daily_no) {
						...
						if(!continueAfterAbort | (continueAfterAbort & !isdone.dailyAggs[doi, sc])){
							...
							if(makeOutputDB) {
								if(agg.analysis == 1){
									#dbWriteTable(con, paste("Aggregation_Seasons_DailyValues_", agg.resp, "_Mean", sep=""), data.frame(out.tempM[1,]), row.names=FALSE, append=TRUE)
									#dbWriteTable(con, paste("Aggregation_Seasons_DailyValues_", agg.resp, "_SD", sep=""), data.frame(out.tempSD[1,]), row.names=FALSE, append=TRUE)
									dbWriteTable(con, "Aggregation_Seasons_DailyValues_Layers", data.frame(t(c(i,sc,paste(agg.resp,"_Mean",sep=""),0,res.dailyMean))), row.names=FALSE, append=TRUE)
									dbWriteTable(con, "Aggregation_Seasons_DailyValues_Layers", data.frame(t(c(i,sc,paste(agg.resp,"_SD",sep=""),0,res.dailyMean))), row.names=FALSE, append=TRUE)
								} else {
									#save(out.tempM, out.tempSD, file=file.path(dir.out, paste(mpi.comm.rank(), "_", sc, "dataForDatabaseDaily.robj", sep="")))
									
									#dbWriteTable(con, paste("Aggregation_Seasons_DailyValues_", agg.resp, "_Mean", sep=""), data.frame(out.tempM[1,1:length(header)]), row.names=FALSE, append=TRUE)
									out.tempM <- subset(out.tempM, select=-(1:length(header)))
									out.tempL <- data.frame(matrix(0, nrow=SoilLayer_MaxNo, ncol=4+366))
									names <- c("RunID", "Scenario", "Type", "Layer", paste("doy", formatC(1:366, width=3, format="d", flag="0"), sep=""))
									colnames(out.tempL) <- names
									out.tempL$Type <- ""
									for(temp_layer in 1:SoilLayer_MaxNo) {
										Lvalues <- out.tempM[1,(1:366)+(366*(temp_layer-1))]
										if(all(is.na(Lvalues))) {
											out.tempL <- out.tempL[1:(temp_layer-1),]
											break
										} else {
											out.tempL[temp_layer,] <- c(header[1],sc, paste(agg.resp,"_Mean",sep=""), temp_layer, Lvalues)
										}
									}
									dbWriteTable(con, "Aggregation_Seasons_DailyValues_Layers", out.tempL, row.names=FALSE, append=TRUE)
									
									#dbWriteTable(con, paste("Aggregation_Seasons_DailyValues_", agg.resp, "_SD", sep=""), data.frame(out.tempSD[1,1:length(header)]), row.names=FALSE, append=TRUE)
									out.tempSD <- subset(out.tempSD, select=-(1:length(header)))
									out.tempL <- data.frame(matrix(0, nrow=SoilLayer_MaxNo, ncol=4+366))
									names <- c("RunID", "Scenario", "Type", "Layer", paste("doy", formatC(1:366, width=3, format="d", flag="0"), sep=""))
									colnames(out.tempL) <- names
									out.tempL$Type <- ""
									for(temp_layer in 1:SoilLayer_MaxNo) {
										Lvalues <- out.tempSD[1,(1:366)+(366*(temp_layer-1))]
										if(all(is.na(Lvalues))) {
											out.tempL <- out.tempL[1:(temp_layer-1),]
											break
										} else {
											out.tempL[temp_layer,] <- c(header[1],sc, paste(agg.resp,"_SD",sep=""), temp_layer, Lvalues)
										}
									}
									dbWriteTable(con, "Aggregation_Seasons_DailyValues_Layers", out.tempL, row.names=FALSE, append=TRUE)
								}
							}#dailyList[[doi]] <- list(M=out.tempM, SD=out.tempSD, name=agg.resp, aggLs_no=aggLs_no)
...
work <- function(parallel_backend, Data) {
	...
	if(!makeOutputDB || (makeOutputDB && !dbSlave)) { #each slave will test this condition and only the first slave will do something else
		junk <- 0
		done <- 0
		while (done != 1) {
...
#Test Concurrent Connection to Database. Footwork was already complete
dbSlave <- FALSE
