...
/*-------------------------------------------
 ...
 -------------------------------------------*/
  ...
  {
    if (size == 0)
    	error(1, "Programmer Error: size == 0 in MallocErr()");
  ...
  p = malloc( size);
    ...
    if( NULL==(f=fopen("memory.log","a")) ) {
      printf("Can't open memory.log for errors\n");
      exit(-1);
    }
    fprintf(f,"%s: %d: %p\n", funcname, size, p);
    fclose(f);
  ...
  if (p == NULL) {
  	printf("Out of memory in %s()", funcname);
    exit(0);
  }
 ...
 -------------------------------------------*/
  ...
  #ifdef DEBUG_MEM
  {
   ...
   if (pNew != NULL) {
       ...
       error(1, "realloc failed in Mem_ReAlloc()");
 ...
 -------------------------------------------*/
  ...
  #ifdef DEBUG_MEM_X
  {
    if (mem_SizeOf(block) > SizeOfMalloc)
      error(1, "Mem: Inconsistency in SizeOfMalloc");
