...
sw_dailyC4_TempVar <- function(dailyTempMin, dailyTempMean, simTime2){
	...
	LengthFreezeFreeGrowingPeriod_Days <- aggregate(dailyTempMin, by=list(simTime2$year_ForEachUsedDay_NSadj), FUN=function(x) max((temp <- rle(x > 0))$lengths[temp$values], na.rm=TRUE))[, 2]
