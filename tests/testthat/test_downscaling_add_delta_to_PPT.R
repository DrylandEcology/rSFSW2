...
test_that("add_delta_to_PPT: calls with argument 'addDelta'", {
  ...
  expect_equal(
    add_delta_to_PPT(data = ppt0, ind_events = rainyElems_int,
      addDelta = delta1a),
    list(data = ppt1, PPT_to_remove = 0))
  # No precip-day index
  expect_equal(
  ...
  # There is enough precipitation from which to subtract when shared among
  # all (already adjusted) rainy days
  expect_equal(
    add_delta_to_PPT(data = ppt0, ind_events = rainyElems, addDelta = delta3a),
    list(data = ppt3, PPT_to_remove = 0))
  # There is not enough precipitation overall from which to subtract -->
  # all days will be 0
  expect_equal(
...
test_that("add_delta_to_PPT: calls with argument 'deltaPerEvent'", {
  ...
  expect_equal(
    add_delta_to_PPT(data = ppt0, ind_events = rainyElems,
      deltaPerEvent = delta1b),
    list(data = ppt1, PPT_to_remove = 0))
  # There are subtractions, but all positive
  expect_equal(
    add_delta_to_PPT(data = ppt0, ind_events = rainyElems,
      deltaPerEvent = delta2b),
    list(data = ppt2, PPT_to_remove = 0))
  # There is enough precipitation from which to subtract when shared among
  # all (already adjusted) rainy days
  expect_equal(
    add_delta_to_PPT(data = ppt0, ind_events = rainyElems,
      deltaPerEvent = delta3b),
    list(data = ppt3, PPT_to_remove = 0))
  # There is not enough precipitation overall from which to subtract
  # --> all days will be 0
  expect_equal(
    add_delta_to_PPT(data = ppt0, ind_events = rainyElems,
      deltaPerEvent = delta4b),
    list(data = ppt4, PPT_to_remove = -1.8))
  # There is enough precipitation from which to subtract when shared among
  # all (already adjusted) rainy days; this uses 3 recursive calls
  expect_equal(
    add_delta_to_PPT(data = ppt0, ind_events = rainyElems,
      deltaPerEvent = delta5b),
    list(data = ppt5, PPT_to_remove = 0))
  # There is not enough precipitation overall from which to subtract
  # --> all days will be 0; this uses 1 recursive calls
  expect_equal(
    add_delta_to_PPT(data = ppt0, ind_events = rainyElems,
      deltaPerEvent = delta6b),
