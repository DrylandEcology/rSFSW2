...
test_that("applyPPTdelta_simple", {
  ...
  expect_equal(
    applyPPTdelta_simple(m = month, data = data0, ydelta = ydelta0, add_days = inotadd,
    mult_days = !inotadd), res0s1)

  # All deltas are additions -> no problem
  expect_equal(
    applyPPTdelta_simple(m = month, data = data0, ydelta = ydelta0, add_days = iadd,
    mult_days = !iadd), res0s2)
  # Deltas are subtractions, but are smaller than precipitation events
  expect_equal(
    applyPPTdelta_simple(m = month, data = data0, ydelta = ydelta1, add_days = iadd,
    mult_days = !iadd), res0s3)
  # Deltas are subtractions, but some are larger than precipitation events causing
  #   negative precipitation
  expect_equal(
    applyPPTdelta_simple(m = month, data = data0, ydelta = ydelta2, add_days = iadd,
    mult_days = !iadd, set_negPPT_to0 = FALSE), res0s4)
  expect_equal(
    applyPPTdelta_simple(m = month, data = data0, ydelta = ydelta2, add_days = iadd,
    mult_days = !iadd, set_negPPT_to0 = TRUE), res0s4nn)
  # Some deltas are additions, some are subtractions
  expect_equal(
    applyPPTdelta_simple(m = month, data = data0, ydelta = ydelta3, add_days = iadd,
    mult_days = !iadd, set_negPPT_to0 = FALSE), res0s5)

  # Some deltas are additions/subtractions, some are multiplications
  expect_equal(
    applyPPTdelta_simple(m = month, data = data0, ydelta = ydelta3, add_days = iadd6,
    mult_days = !iadd6, set_negPPT_to0 = FALSE), res0s6)
})
