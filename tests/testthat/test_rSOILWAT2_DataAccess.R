...
#--- Tests
test_that("set_requested_rSOILWAT2_InputFlags:", {
  #--- INPUTS
  path_demo <- system.file("extdata", "example1", package = "rSOILWAT2")
  rswin <- rSOILWAT2::sw_inputDataFromFiles(dir = path_demo, files.in = "files.in")

  tasks <- list(create = 1L)
  ...
  #--- TESTS
  for (test in tests) {
    args <- c(test_args[[test]], list(swIn = rswin), list(tasks = tasks))

    # 1) Check: all use turned off
    dat <- test_data[[test]]

    expect_silent(temp <- do.call(set_requested_rSOILWAT2_InputFlags, args = c(args, dat)))
    expect_equal(temp[["swIn"]], args[["swIn"]], info = test)
    expect_equal(temp[["tasks"]], args[["tasks"]], info = test)
    ...
    for (k in 1:2) {
      ...
      if (isTRUE(length(dim(dat[["values"]])) > 1)) {
        ...
        dat[["use"]][1] <- TRUE
        dat[["values"]][1] <- NA
        expect_output(temp <- do.call(set_requested_rSOILWAT2_InputFlags,
          args = c(args, dat)))
        expect_equal(temp[["swIn"]], args[["swIn"]], info = test)
        expect_equal(temp[["tasks"]], list(create = 0), info = test)
        ...
        expect_silent(temp <- do.call(set_requested_rSOILWAT2_InputFlags,
          args = c(args, dat)))
        values <- utils::getFromNamespace(test_args[[test]][["fun"]], "rSOILWAT2")(temp[["swIn"]])
    ...
    dat[["use"]][1] <- TRUE
    dat[["values"]][1] <- NA
    expect_output(temp <- do.call(set_requested_rSOILWAT2_InputFlags, args = c(args, dat)))
    expect_equal(temp[["swIn"]], args[["swIn"]], info = test)
    expect_equal(temp[["tasks"]], list(create = 0), info = test)
  }
})
