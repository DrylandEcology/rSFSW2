...
#   - dailySuitablePeriodsAvailableWater, dailySWPdrynessIntensity, monthlySWAbulk
#     - case 1: SWPtoVWC(SWPcrit_MPa[icrit], texture$sand.top, texture$clay.top)
#   - mean doy: SWAbulk
#     - case 2: SWPtoVWC(index.SWPcrit, sand, clay)
#   - mean doy: SWPmatric
#     - case 3: VWCtoSWP(res.dailyMean[ir], textureDAgg$sand[al], textureDAgg$clay[al]) # ir is vector; al is value
# section: functions
#   - get_SWPmatric_aggL
#     - case 3: VWCtoSWP(vwcmatric$top, texture$sand.top, texture$clay.top)
#     - case 3: VWCtoSWP(vwcmatric$aggMean.top, texture$sand.top, texture$clay.top)
#     - case 6: VWCtoSWP(vwcmatric$val[, -index.header], sand, clay)


# Inputs
# Table 2 from Cosby, B.J., Hornberger, G.M., Clapp, R.B. & Ginn, T.R. (1984). A statistical exploration of the relationships of soil moisture characteristics to the physical properties of soils. Water Resour Res, 20, 682-690.
texture <- data.frame(
...
row.names(texture) <- c("Sand", "Loamy sand", "Sandy loam", "Loam", "Silty loam",
...
if (FALSE) {
  ...
  dimnames(swp_vals) <- list(row.names(texture), names(swp_fix), row.names(texture))
  saveRDS(swp_vals, "swp_values.rds")

} else {
  swp_vals <- readRDS("swp_values.rds")
}

#--- Tests
test_that("To SWP", {
  # 1. VWC in fraction [single value] + sand and clay in fraction [single values]
  #    --> SWP in MPa [single value]
  for (ifix in names(swp_fix)) for (itext in row.names(texture))
    expect_equivalent(
      VWCtoSWP(vwc_fix[itext, ifix], texture[itext, "sand"], texture[itext, "clay"]),
      swp_fix[ifix])

  # 2. VWC in fraction [single value] + sand and clay in fraction [vectors of length d]
  ...
  # 3. VWC in fraction [vector of length l] + sand and clay in fraction [single values]
...
test_that("To VWC", {
  ...
  # 2. SWP in MPa [single value] + sand and clay in fraction [vectors of length d]
  ...
  # 3. SWP in MPa [vector of length l] + sand and clay in fraction [single values]
  #    --> VWC in fraction [vector of length l]
  for (ifix in names(swp_fix)) for (itext in row.names(texture))
    expect_equivalent(
      SWPtoVWC(rep(swp_fix[ifix], nrow(texture)), texture[itext, "sand"], texture[itext, "clay"]),
