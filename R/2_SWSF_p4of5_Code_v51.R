...
if (usePreProcessedInput && file.exists(file.path(dir.in, datafile.SWRWinputs_preprocessed))) {
  ...
  site_labels <- SWRunInformation$Label
...
if (do_check_include) {
	...
	if (length(includes_sources) > 0L) {
		...
		if (all(include_YN_sources[include_YN])) {
			if (!be.quiet)
...
#i_sim: a value of runIDs_total, i.e., index through the simulation runs
...
#------------------------CREATE RUNS
    ...
    do_vegs <- list(
      ...
      if (any(create_treatments == do_vegs[["flag"]][k])) {
        ...
        if (any(temp1)) {
          print(paste("ERROR: albedo column(s)", paste(
...
if (FALSE) {
          ...
          outputs_by_layer <- outputs_by_layer + ifelse(hydred < 0, -hydred, 0)
            ...
            res2 <- apply(x[, soilLayers_N + ld, drop = FALSE], 2, function(y) list(out = y))
          ...
          if (!is.matrix(extraction_duration_days)) {
            extraction_duration_days <- matrix(extraction_duration_days, nrow = soilLayers_N, ncol = simTime$no.useyr)
          }
          ...
          if (!is.matrix(extraction_summed_mm)) {
            extraction_summed_mm <- matrix(extraction_summed_mm, nrow = soilLayers_N, ncol = simTime$no.useyr)
          }

					# aggregate across years for each soil layer
					resAgg[nv:(nv+soilLayers_N-1), ] <- t(round(apply(extraction_duration_days, 1, agg_fun, na.rm = TRUE), 1))
					nv <- nv+SoilLayer_MaxNo
					resAgg[nv:(nv+soilLayers_N-1), ] <- t(round(apply(extraction_summed_mm, 1, agg_fun, na.rm = TRUE), 1))
					nv <- nv+SoilLayer_MaxNo
...
#TODO(drs): progress state
			#37
				if(aon$duration_min_drysoils_days){#Dry and wet periods based on daily swp: accountNSHemispheres_agg
...
nv <- 1
          if (!exists("dry_soil_all_dy")) {
            dry_soil_all_dy <- soil_status(
              top = swpmatric.dy.all$val[simTime$index.usedy, 2 + topL, drop = FALSE],
              bottom = swpmatric.dy.all$val[simTime$index.usedy, 2 + bottomL, drop = FALSE],
              swp_crit = SWPcrit_MPa, time_N = simTime$no.usedy, is_dry = TRUE)
          }
          ...
          tx[["AtLeastOneDry"]] <- vapply(dry_soil_dy, function(x) apply(x, 1, any),
            FUN.VALUE = rep(NA, simTime$no.usedy))
...
resMeans[1:(nv - 1)]
          ...
          if (!exists("wet_soil_dy")) {
            wet_soil_dy <- soil_status(top = swpmatric.dy$top, bottom = swpmatric.dy$bottom,
              swp_crit = SWPcrit_MPa, time_N = simTime$no.usedy, is_dry = FALSE)
          }
          ...
          if (!exists("adj_NS_days")) {
            adj_NS_days <- season_diff_NS(simTime2, t_unit = "day")
					...
					for(icrit in seq(along=SWPcrit_MPa)){
						...
						temp <- aggregate(cbind(dry.top, dry.bottom), by=list(simTime2$year_ForEachUsedDay_NSadj), FUN=function(x) c(if(any((temp <- rle(x))$values)) c(mean(temp$lengths[temp$values]), max(temp$lengths[temp$values])) else c(0, 0), sum(x), startDoyOfDuration(x, duration=durationDryPeriods.min) - adj_NS_days))
						...
						resSDs[nv:(nv+7)] <- c(apply(temp$dry.top[, 1:3, drop=FALSE], 2, sd), circ_sd(x=temp$dry.top[, 4], int=365), apply(temp$dry.bottom[, 1:3, drop=FALSE], 2, sd), circ_sd(x=temp$dry.bottom[, 4], int=365))
						nv <- nv+8
					}

					rm(dry.top, dry.bottom, suitable, dry_crit, durationDryPeriods.min)
				}
			#41
...
if(actionWithSoilWat && runsN_todo > 0){
  ...
  list.export <- c("accountNSHemispheres_agg", "accountNSHemispheres_veg",
    "adjust.soilDepth", "agg_fun_defs", "agg_windows", "agg_fun", "agg_fun_circular",
    "aon", "be.quiet", "bin.prcpfreeDurations", "bin.prcpSizes",
    ...
    "do_OneSite", "do.GetClimateMeans", "done_prior", "duration_min_drysoils_days", "endyr", "estabin",
    ...
    "print.debug", "prodin", "runsN_master",
    "runsN_job", "saveRsoilwatInput", "saveRsoilwatOutput", "scenario_No",
  ...
  obj2exp <- gather_objects_for_export(
	...
	#ETA calculation
	if (!be.quiet)
	  print(paste("SWSF simulation runs:", runsN_todo, "out of", runsN_job,
	    "runs will be carried out on", workersN, "cores: started at", t1 <- Sys.time()))
...
#			assign(x="i_labels",value=site_labels[i_site],envir=exeEnv)
