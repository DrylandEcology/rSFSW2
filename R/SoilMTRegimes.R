...
#'   function \code{\link{setup_simulation_time}}.
...
dailyNRCS_SoilMoistureTemperatureRegimes <- function(
  ...
  years <- seq(rSOILWAT2::swYears_StartYear(sim_in),
  ...
  if (is_simTime1_good) {
    ...
    st1 <- setup_simulation_time(
      sim_time = list(
        simstartyr = years[1], startyr = years[1],
        endyr = years[length(years)],
        future_yrs = matrix(NA, nrow = 0, ncol = 3)),
      add_st2 = FALSE)
  }
  ...
  if (is_simTime2_good) {
    ...
    st2 <- rSOILWAT2::simTiming_ForEachUsedTimeUnit(useyrs = years,
      sim_tscales = c("daily", "monthly", "yearly"),
      latitude = rSOILWAT2::swSite_IntrinsicSiteParams(sw_in)[["Latitude"]],
  ...
  if (has_soil_temperature) {
    ...
    if (!exists("soiltemp.dy.all", where = sim_agg)) {
      sim_agg[["soiltemp.dy.all"]] <- list(val =
          slot(slot(sim_out, swof["sw_soiltemp"]), "Day"))
    }
    ...
    if (!anyNA(sim_agg[["soiltemp.dy.all"]][["val"]]) &&
      ...
      if (!exists("soiltemp.yr.all", where = sim_agg)) {
        sim_agg[["soiltemp.yr.all"]] <- list(val =
          slot(slot(sim_out, swof["sw_soiltemp"]), "Year"))
      }
      if (!exists("soiltemp.mo.all", where = sim_agg)) {
        sim_agg[["soiltemp.mo.all"]] <- list(val =
          slot(slot(sim_out, swof["sw_soiltemp"]), "Month"))
      }
      if (!exists("vwcmatric.dy.all", where = sim_agg)) {
        sim_agg[["vwcmatric.dy.all"]] <- list(val =
          slot(slot(sim_out, swof["sw_vwcmatric"]), "Day"))
      }
      if (!exists("swpmatric.dy.all", where = sim_agg)) {
        sim_agg[["swpmatric.dy.all"]] <- get_SWPmatric_aggL(
          vwcmatric = sim_agg[["vwcmatric.dy.all"]], texture = NULL,
          sand = soildat[, "sand_frac"], clay = soildat[, "clay_frac"])
      }

      if (!exists("prcp.yr", where = sim_agg)) {
        sim_agg[["prcp.yr"]] <- get_PPT_yr(sim_out, st1)
      }
      if (!exists("prcp.mo", where = sim_agg)) {
        sim_agg[["prcp.mo"]] <- get_PPT_mo(sim_out, st1)
      }
      if (!exists("pet.mo", where = sim_agg)) {
        sim_agg[["pet.mo"]] <- get_PET_mo(sim_out, st1)
      }
      if (!exists("temp.mo", where = sim_agg)) {
        sim_agg[["temp.mo"]] <- get_Temp_mo(sim_out, st1)
      }
      ...
      if (SMTR[["SMR_normalyears_N"]] > 0) {
        ...
        swp_dy_nrsc <- if (swp_recalculate || opt_SMTR[["aggregate_at"]] == "data") {
          get_SWPmatric_aggL(vwc_dy_nrsc, texture = NULL,
            sand = soildat[, "sand_frac"], clay = soildat[, "clay_frac"])
        } else {
          sim_agg[["swpmatric.dy.all"]]
        }
        swp_dy_nrsc <- swp_dy_nrsc[["val"]][
          st_NRCS[["index_usedy"]], -(1:2), drop = FALSE]
        ...
        temp <- cbind(rSOILWAT2::swProd_MonProd_grass(sim_in)[, "Litter"],
        ...
        crit_litter <- crit_Oh[3] *
          sum(rSOILWAT2::swProd_Es_param_limit(sim_in) * veg_comp)
