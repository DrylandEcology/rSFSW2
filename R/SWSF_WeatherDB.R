...
make_dbW <- function(dbWeatherDataFile, runIDs_sites, SWRunInformation, simstartyr, endyr,
    ...
    dir.ex.Livneh = NULL, verbose = FALSE) {
  ...
  if (ids_Livneh_extraction > 0)) {
    extract_daily_weather_from_livneh(dir_data = dir.ex.Livneh, dir_temp = dir.out.temp,
                                      ...
                                      coords = SWRunInformation[ids_Livneh_extraction, c("X_WGS84", "Y_WGS84"), drop = FALSE],
                                      start_year = simstartyr, end_year = endyr,
                                      comp_type = dbW_compression_type,
                                      run_parallel = parallel_init && parallel_runs && identical(parallel_backend, "cluster"),
                                      num_cores = num_cores, be_quiet = be.quiet)
  }
