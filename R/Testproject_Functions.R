...
#'   between test and reference databases. A first entry is the file name of the
#'   reference database.
#'
...
compare_test_output <- function(dir_test, dir_ref = NULL, tol = 1e-3,
  ...
  refDB <- RSQLite::dbConnect(RSQLite::SQLite(),
  ...
  if (any(!testDB_tables_comp)) {
    diff_msgs <- c(diff_msgs, paste("Test database contains tables without an",
      "analog in the reference database:",
  ...
  if (any(!refDB_tables_comp)) {
    diff_msgs <- c(diff_msgs, paste("Reference database contains tables",
      "without an analog in the test database:",
  ...
  for (k in seq_along(tocomp_tables)) {
    ...
    if (any(!testDB_fields_comp)) {
      diff_msgs <- c(diff_msgs, paste("Test database table",
        shQuote(tocomp_tables[k]), "contains fields without an analog in the",
        "reference database:",
