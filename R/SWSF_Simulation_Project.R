...
init_project <- function(project_paths, fnames_in, opt_verbosity) {
 ...
 list(ow_prev = ow_prev, fnames_in = fnames_in)
}
...
#' @export
setup_rSWSF_project_infrastructure <- function(project_paths, opt_verbosity) {
  if (opt_verbosity[["verbose"]])
    print(paste("A new rSWSF project is prepared for:",
      sQuote(basename(project_paths[["dir_prj"]])), "and started at", Sys.time()))
  ...
  for (di in definf) {
    dtemp <- file.path(project_paths[["dir_prj"]], di[["path"]])
  ...
  if (opt_verbosity[["verbose"]])
    print(paste("The new rSWSF project was successfully prepared at:",
      sQuote(project_paths[["dir_prj"]])))
...
#------------------------PREPARE SOILWAT2 SIMULATIONS
  ...
  temp <- init_project(project_paths, fnames_in, opt_verbosity)
  ow_prev <- temp[["ow_prev"]]
  fnames_in <- temp[["fnames_in"]]
