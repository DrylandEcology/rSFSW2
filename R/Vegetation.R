...
#'   argument(s).
#'
#' @section Note: The remaining vegetation types, i.e., shrubs, C3 grasss, C4
...
#' @param dailyC4vars A named numeric vector of length 3 or \code{NULL}.
    ...
    if (MAP_mm <= 1) {
      ...
      if (MAT_C <= 0) {
        ...
        if (!is.null(dailyC4vars)) {
          ...
          if (grass_c4_species <= SFSW2_glovars[["tol"]]) {
            estim_cover[igc4] <- 0
          }
        }
      }
