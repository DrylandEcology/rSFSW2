...
#	= "create": puts information and files together for each simulation SOILWAT2 run
#	= "execute": executes the SOILWAT2 simulations
#	= "aggregate": calculates aggregated response variables from the SOILWAT2 output
...
#								- if actions does not include "create", then code assumes that all SOILWAT2-runs are pre-prepared in the directory 'dir_out_sw' (5. source);
#								  i.e., code obtains the list of simulation runs from 'datafile.SWRunInformation' and executes and/or aggregates the runs
#	= "folders":	assumes that all SOILWAT2-runs are pre-prepared in the directory 'dir_out_sw' (5. source); 'actions' == "create" is ignored
...
#		- added flag 'deleteSoilWatOutputAfterAggregation': deletes all SOILWAT2 simulation output after aggregation for run is complete, but maintains the SOILWAT2 input file structure; this safes space (e.g., for 1000 years of complete output = 490 MB); if other response variables are needed, then only actions==c("execute", "aggregate") are needed, but not "create"
