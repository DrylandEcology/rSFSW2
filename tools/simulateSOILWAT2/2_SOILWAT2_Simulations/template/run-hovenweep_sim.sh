...
#SBATCH --job-name=rms
#SBATCH --output=logs/%j_log-slurm_pipofm-sim.txt
...
srun date
srun Rscript SFSW2_project_code.R -nparallel=${SLURM_CPUS_ON_NODE} > logs/$(date +%Y%m%d-%H%M%S)_log-hw_rSFSW2.txt 2>&1
srun date
